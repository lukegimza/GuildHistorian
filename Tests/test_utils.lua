...
describe("Utils.HashKey", function()
    it("should return a number", function()
        A.isNumber(ns.Utils.HashKey("test"))
    end)

    it("should return the same hash for the same input", function()
        local h1 = ns.Utils.HashKey("hello world")
        local h2 = ns.Utils.HashKey("hello world")
        A.equals(h1, h2)
    end)

    it("should return different hashes for different inputs", function()
        local h1 = ns.Utils.HashKey("hello")
        local h2 = ns.Utils.HashKey("world")
        A.notEquals(h1, h2)
    end)

    it("should handle empty string", function()
        local h = ns.Utils.HashKey("")
        A.isNumber(h)
        A.equals(5381, h, "djb2 of empty string should be seed value 5381")
    end)

    it("should stay within 32-bit range", function()
        local h = ns.Utils.HashKey("a very long string that should still produce a bounded hash value 1234567890")
        A.isTrue(h >= 0 and h < 2147483647)
    end)

    it("should handle special characters", function()
        A.isNumber(ns.Utils.HashKey("test-123:special!@#$%^&*()"))
    end)
end)

describe("Utils.BuildDedupKey", function()
    it("should produce a numeric key", function()
        local key = ns.Utils.BuildDedupKey("BOSS_KILL", 1700000000, "123", "16")
        A.isNumber(key)
    end)

    it("should produce same key for same inputs", function()
        local k1 = ns.Utils.BuildDedupKey("BOSS_KILL", 1700000000, "Ragnaros")
        local k2 = ns.Utils.BuildDedupKey("BOSS_KILL", 1700000000, "Ragnaros")
        A.equals(k1, k2)
    end)

    it("should produce different keys for different event types", function()
        local k1 = ns.Utils.BuildDedupKey("BOSS_KILL", 1700000000, "Ragnaros")
        local k2 = ns.Utils.BuildDedupKey("FIRST_KILL", 1700000000, "Ragnaros")
        A.notEquals(k1, k2)
    end)

    it("should produce different keys for different timestamps", function()
        local k1 = ns.Utils.BuildDedupKey("BOSS_KILL", 1700000000, "Ragnaros")
        local k2 = ns.Utils.BuildDedupKey("BOSS_KILL", 1700000001, "Ragnaros")
        A.notEquals(k1, k2)
    end)

    it("should handle nil extra fields gracefully", function()
        local key = ns.Utils.BuildDedupKey("BOSS_KILL", 1700000000, nil, nil)
        A.isNumber(key)
    end)

    it("should handle no extra fields", function()
        local key = ns.Utils.BuildDedupKey("LOOT", 1700000000)
        A.isNumber(key)
    end)
end)
